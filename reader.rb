  ...
  class Reader
    ...
    def initialize(filename) 
      File.open(filename, "rb") do |f|
        read_bmp_header(f) # does some validations
        read_dib_header(f) # sets @width, @height
        read_pixels(f)     # populates the @pixels array
      end
    end
    ...
    def read_bmp_header(file)
      header = file.read(14)
      magic_num, file_size, reserved1,
      reserved2, array_location = header.unpack("A2Vv2V")
      
      fail "Not a bitmap file!" unless magic_num == "BM"
    ...
    def read_dib_header(file)
      ...
      unless image_size + PIXEL_OFFSET == file.size
